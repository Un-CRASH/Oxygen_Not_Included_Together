...
namespace ONI_Together.Networking.Synchronization
{
	...
	public static class TerrainReconcile
	{
		...
		public static void OnRequest(TerrainChunkRequestPacket packet)
		{
			...
			if (sent > 0)
				DebugConsole.LogWarning($"[TerrainReconcile] sent {sent} terrain chunk(s) to {packet.SenderId} whose terrain differs there");
		}
		...
		public static void OnDigest(TerrainDigestPacket packet)
		{
			...
			DebugConsole.LogWarning($"[TerrainReconcile] terrain differs from the host in chunk(s) {string.Join(", ", request.Chunks.Select(c => $"({c.Item1},{c.Item2})"))}; asking for the host's cells");
